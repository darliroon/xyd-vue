<template>
  <div>
    ...
    <el-form ref="loginForm" :model="form" :rules="rules" label-width="80px" class="login-box">
      <h3 class="login-title">New Main Objects</h3>
...
export default {
  ...
  data () {
    return {
      form: {
        tableName: this.$route.query.finger_name,
        all_args: 'name text, coordinate text, label int, img mediumblob, img_url text, details text, comments text'
      },
  ...
  methods: {
    onSubmit (formName) {
      this.form.tableName = this.$route.query.finger_name
      this.form.all_args = 'name text, coordinate text, label int, img mediumblob, img_url text, details text, comments text'
      ...
      this.$axios({
        url: 'http://127.0.0.1:5000/newObjectTable',
        method: 'post',
        // 发送格式为json
        ...
        if (res['data'] === 0) {
          ...
          this.$router.push({
            path: this.$route.path + '/mode2_',
            query: {
              total_name: this.$route.query.finger_name,
