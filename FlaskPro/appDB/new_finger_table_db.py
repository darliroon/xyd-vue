...
class NewTable:
    def __init__(self, host='rm-j6c1pi4d3j4u787x3mo.mysql.rds.aliyuncs.com', user='thl_admin', password="Ll5028852", db="xydinfo", db2="xydposition"):
        self.host = host
        self.user = user
    ...
    def create_new_table(self, new_table_name, create_sql):
        sql = "SELECT name FROM positiontableinfo"
        ...
        try:
            ...
            self.conn2.commit()
            insert_sql = "INSERT INTO positiontableinfo VALUES (%s)"
    ...
    def drop_the_table(self, table_name):
        ...
        for (_name,) in res:
            if _name == table_name:
                try:
                    delete_sql = "DELETE FROM positiontableinfo WHERE name = %s"
